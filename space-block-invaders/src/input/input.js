...
class Input{
    ...
    tick(game){
        ...
        if (gp != null){
            ...
            if (game.keys['KeyD'] == "keydown" || game.keys['ArrowRight'] == "keydown") this.axes.x = 1;
            if (game.keys['KeyA'] == "keydown" || game.keys['ArrowLeft'] == "keydown") this.axes.x = -1;
            if (game.keys['KeyS'] == "keydown" || game.keys['ArrowDown'] == "keydown") this.axes.y = 1;
            if (game.keys['KeyW'] == "keydown" || game.keys['ArrowUp'] == "keydown") this.axes.y = -1;
            if (game.keys['Space'] == "keydown") this.firePressed = true;
            // usePressedPreviously is to stop the usekey to spam a new click when holding it down
            if (game.keys['KeyE'] == "keydown" && !this.usePressedPreviously) this.usePressed = true;
            this.usePressedPreviously = game.keys['KeyE'] == "keydown";
        }
    }
}
