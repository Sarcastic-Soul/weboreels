...
void main() {
    ...
    // Based on: https://github.com/carlini/js13k2020-yet-another-space-shooter/blob/master/src/graphics.js#L216-L228
        if (v_color.r < 0.1 && v_color.g < 0.1 && v_color.b < 0.1) {
            vec3 skyPosition = normalize(normalize((-v_position).xyz)-.5);
            float skyPrevLength, skyLength = skyPrevLength = 0.;
            for (int skyIteration=0; skyIteration<15; skyIteration += 1) {
                skyPosition = abs(skyPosition) / dot(skyPosition, skyPosition) - .49;
                skyLength += abs(length(skyPosition) - skyPrevLength);
                skyPrevLength = length(skyPosition);
      }
            skyLength *= skyLength * skyLength;
            vec3 skyColor = pow(vec3(skyLength/2e5, skyLength/2e5, skyLength/1e5), vec3(.9));
            outputColor.rgb = .5 * clamp(skyColor,0.,2.) + vec3(0.03, 0.0, 0.12);
      outputColor.a = 1.0;
      return;
    }
