...
initGame();
registerSong('Milky Way', milkyWaySong);
...
function handleInput() {
  ...
  if (gameState !== GameState.AFTER_LEVEL && gameState !== GameState.ENDING) {
...
function createJetpackParticles() {
  if (Math.random() < 0.3) {
    ...
    blast.size = 0.2;
    blast.velocity[1] = (gameState === GameState.AFTER_LEVEL || gameState === GameState.ENDING) ? -10 : -0.3;
